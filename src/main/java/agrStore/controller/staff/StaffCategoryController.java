...
public class StaffCategoryController {
	...
	public String categoryManagement() {
		return "staff/category/categoryManagement";
	}
	...
	public String handleCategory(@RequestParam(value = "action", required = false) String action,
		...
		return "staff/category/categoryForm"; // Trả về cùng một trang JSP
	}
	...
	public String addCategory(HttpServletRequest request, Model model,
		...
		if (!isValid) {
			...
			model.addAttribute("mode", "ADD");
			return "staff/category/categoryForm";
		} else {
			try {
				...
				return "redirect:/staff/categoryManagement.htm";
			} catch (Exception e) {
				e.printStackTrace();
				System.out.println("Error: New category add failed!");
				model.addAttribute("mode", "ADD");
